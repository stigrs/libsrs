...
public:
    ...
    // Return n first elements.
    Array_ref<T, 1> head(size_type n) { return slice(0, n - 1); }
    Array_ref<const T, 1> head(size_type n) const { return slice(0, n - 1); }

    // Return n last elements.
    Array_ref<T, 1> tail(size_type n) { return slice(size() - n, size() - 1); }
    Array_ref<const T, 1> tail(size_type n) const
    {
        return slice(size() - n, size() - 1);
    }
